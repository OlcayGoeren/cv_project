...
class BodyDataset(Dataset):
    def __init__(self, dir, x_dataset, transform=None):
    ...
    def __getitem__(self, index):
        image = np.array(Image.open(os.path.join(
            self.dir + 'images/', self.x_dataset[index])).convert("RGB"))
        mask = np.array(Image.open(os.path.join(
            self.dir + 'masks/', self.x_dataset[index])).convert("L"))
        return image, mask
...
train_train_dataset = BodyDataset(x_dataset=train_dataset, dir=DATA_DIR)
valid_train_dataset = BodyDataset(x_dataset=test_dataset, dir=DATA_DIR)
